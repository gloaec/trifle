...
import trifle.raft.store as store
import trifle.raft.tcp as channel
import trifle.raft.log as log


def make_server(port=9289, bootstraps=[]):
    queue = Queue.Queue()
...
class Server(threading.Thread):
    ...
    def run(self):
        self.running = True
        sfile = os.path.join(config['TMP_DIR'], "raft-log-%d" % self.port)
    ...
    def handle_message(self, msg, addr):
        ...
        try:
            ...
            return
        sfile = os.path.join(config['TMP_DIR'], "raft-log-%d" % self.port)
        with open(sfile, 'a') as w:
            w.write('handle_message(%s, %s)\n' % (msg, addr))
        mtype = msg['type']
