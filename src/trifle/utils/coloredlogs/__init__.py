...
class ColoredStreamHandler(logging.StreamHandler):
    ...
    def __init__(self, stream=sys.stderr, level=logging.NOTSET, isatty=None, \
            show_name=True, show_severity=True, show_timestamps=True, show_hostname=False):
    ...
    def emit(self, record):
        ...
        color = 'white'
        bold  = False
        if severity == 'CRITICAL' : color = 'red' ; bold  = True
        elif severity == 'ERROR'  : color = 'red' 
        ...
        if self.show_timestamps:
            parts.append(self.wrap_color('black', self.render_timestamp(record.created), bold=True))
        ...
        if self.show_severity:
            parts.append(self.wrap_color(color, severity, bold=True))
