...
framework conceptuel dérivé par couches. Nous présenterons une certaine variété
...
gens comprennent de manière tacite ce qu'est le contexte, ils le trouvent par
...
Le contexte physique d'un autre coté peut se traduire par la présence d'une
...
\begin{enumerate}
    \item \textbf{Les informations de contexte présentent une gamme de
        ...
        l'environnement auquel elle appartient : virtuel ou physique ; elle
        peut en outre être subdivisée selon un critère de temporalité :
...
La méthode d'acquisition des données de contexte est très importante lorsque
l'on conçoit un système de cette nature, puisque c'est ce qui prédéfinit le
style architectural du système. Chen (2003) \cite{chen_intelligent_2003}
présente trois approches différentes pour acquérir l'information de contexte.
...
L'approche intergicielle introduit une architecture en couches avec l'intention
...
Cette approche distribuée élargit l'infrastructure basée sur les intergiciels en
...
\begin{itemize}
    \item \textbf{Widgets} -
        ...
        totalement indépendamment de l'application, ce qui permet à
...
\begin{itemize}
    ...
    \item \textbf{Extensibilité} : 
        %\begin{quotation}
            ...
            être facilement extensibles pour accueillir toute nouvelle source
...
\cite{strang_context_2004}, basée sur six critères d'exigences, montre que les
ontologies incarnent de loin le modèle le plus expressif et comblent la plupart
de ces exigences.
...
Un atome de contexte peut être décrit à l'aide de quelques attributs seulement.
...
Comme le contexte est susceptible de contenir des informations sensibles sur des
...
d'autres utilisateurs. Néanmoins, en ce qui concernent les sondes capables de
déterminer la présence d'individus dans une pièce ou à tout autre endroit, il
n'existe aucunes préconisations à ce sujet. C'est un point toutefois critique.
D'une manière générale, dans un environnement mettant à disposition un certain
...
Le tableau \ref{table:comparison_frameworks} met en concurrence les
...
\begin{table*}%[h]
    ...
    \label{table:comparison_frameworks}
\end{table*}
...
Afin d'obtenir des applications et des services sensibles au contexte
...
contenir aucune sémantique restreint de manière considérable le développement
...
données de contexte, mais aucun d'entre eux ne les exploite vraiment.
...
Les aspects liés à la sécurité ne sont pas implémentés dans les autres
...
Chacun des systèmes et frameworks présentés dans cet état de l'art implémente
...
sophistication des systèmes distribués fait leur force de calcul, leur
extrême complexité est souvent la cause de leur boycott. Le modèle issu de
